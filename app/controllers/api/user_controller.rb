class Api::UserController < ApplicationController
  ...
  def logout_clef
    ...
    if(response["success"])
      ...
      if(user = User.where(clef_id: clef_id).first) then
        logger.debug(clef_id)
        logger.debug(user.email)
        EM.run{
        client = Faye::Client.new("http://localhost:8008/faye")
        client.add_extension(ClientAuth.new)
        publication = client.publish('/userSessionInfo/' + user.id, :action => "sign_out")
        }
    end
    else
  ...
  def launchkey_newuser
    ...
    until (LaunchKey.poll_request(auth_request))
        sleep(1)
    end
  ...
  def create
    ...
    clean_params.delete(:subscriptions_video_ids)
    clean_params.delete(:subscription_channel_ids)
  ...
  def user_params
    new_params = params
    new_params[:user].delete(:subscriptions_video_ids)
    new_params[:user].delete(:subscription_channel_ids)
    new_params.require(:user).permit!
  end
end
